...
class DINOloss(nn.Module):
    ...
    def forward(
        self, teacher_outs, student_outs, current_epoch
    ):  # [bs * num_views, out_dim]

        student_outs = student_outs / self.s_temp
        teacher_outs = F.softmax(
            (teacher_outs - self.center) / self.t_temp[current_epoch], dim=-1
        )

        student_outs = torch.chunk(
            student_outs, chunks=(self.num_views[0] + self.num_views[1])
        )
    ...
    def update_center(self, teacher_outs):
        ...
        self.center = (
            self.center * self.center_momentum + (1 - self.center_momentum) * center
        )
